...
\ * CHANGES     : March 9th, 2003  J.v.d.Ven: Changed for fmacro.f
\ *               March 10th, 2003 J.v.d.Ven: Changed DDOT and DO_DAXPY
...
ALL-TESTS   \ Using the orginal code and Win32Forth Version: 6.09
CLK 400 MHz
...
CLK 400 MHz
80x80 mm - normal algorithm                             48.76 MFlops,   8.20 ticks/flop,   0.020 s
80x80 mm - blocking, factor of 20                       19.22 MFlops,  20.80 ticks/flop,   0.053 s
80x80 mm - transposed B matrix                          35.08 MFlops,  11.40 ticks/flop,   0.029 s
...
NEEDS fmacro.f
NEEDS fsl_util.f  ' FSL-Utilities_1.04 drop
...
: F<>   ( F: r -- ) ( -- bool ) F= 0= ;
: DFVARIABLE ( -- ) qalign CREATE 8 ALLOT ;

0 CONSTANT U>D

also assembler definitions SYS-WARNING-OFF

: DF!+          ( addr -- addr' ) ( hw: r -- )   DUP DF! DFLOAT+ ;
: DF+!          ( addr -- addr' ) ( hw: r -- )   DUP DF@ F+ DF!  ;
: DF@+          ( addr -- addr' ) ( hw: -- r )   DUP DF@ DFLOAT+ ;
: DFLOAT[]      ( addr ix -- addr' )             DFLOATS + ;


only forth also definitions SYS-WARNING-ON


CODE DFLOAT[]      ( addr ix -- addr' )         DFLOATS +   next, end-code


: DF+!  ( addr -- addr' ) ( F: r -- ) DUP DF@ F+ DF!  ;
: DF!+  ( addr -- addr' ) ( F: r -- ) DUP DF! DFLOAT+ ;
: DF+!+ ( addr -- addr' ) ( F: r -- ) DUP DF@ F+ DF!+ ;
...
variable inc2           variable inc1
0e fvariable fzero      fzero f!

32align \ See the note at DO-MAENO
 code  DDOT ( addr addr1 inc1 addr2 inc2 count -- )
         SWAP DFLOATS  >R  ROT DFLOATS R>   inc2 addr!  inc1 addr!
         b>r -ROT a! b!
         fzero faddr@ 0 ass-lit
         2DUP <>
                    IF
                           DO      b@ DF@  inc1 @to_b+
                                   a@ DF@  inc2 @to_a+
...
code DAXPY ( addr1 inc1 addr2 inc2 count -- ) ( F: r -- r' )     \ 242
        SWAP DFLOATS >R  ROT DFLOATS R>  fs>hw inc2 addr!  inc1 addr!
        b>r -rot a! b!
        0 ass-lit
        2DUP <>
             IF
                        DO      b@ FDUP  DF@  F*  inc1 @to_b+
                                a@ DF+!           inc2 @to_a+
...
: NORMAL() ( -- )
        ...
        N 0 ?DO c{{ I 0 }}
                ...
                ?DO  [S] 1  I N  N DDOT  DFLOAT1 +LOOP
                DROP
           LOOP
        .RESULT  ;
...
: TRANSPOSE() ( -- )
        ...
        N 0 ?DO  c{{ I 0 }}  N 0 ?DO  a{{ J 0 }} 1  bt{{ I 0 }} 1  N  DDOT   LOOP DROP LOOP
        .RESULT ;
...
: ROBERT() ( -- )
        ...
        N 0 ?DO   bt{{ 0 0 }}
                   c{{ I 0 }}
                  N 0 ?DO  [S] 1  3 PICK 1  N  DDOT    SWAP N DFLOAT[] SWAP  LOOP
                  2DROP
                  N DFLOATS [S] + TO [S]
           LOOP
        .RESULT ;
...
32align
code do-WARNER()     ( a b - a' b' )
        b>r a!  b!
        a@  DF@  b@ DF@+ F* addr s00 DF+!
        a@  DF@  DF@  F* addr s01 DF+!
        a@   'n addr@ DFLOAT[] DUP  DF@   b@ DF@+ F* addr s10 DF+!
        SWAP DF@  DF@  F* addr s11 DF+!
        DFLOAT1 ass-lit   a@+
        'n addr@ DFLOATS  b@+  swap
        r>b
   next,
   end-code
                  ...
                  N 0 ?DO I TO jj
                          ...
                          N 0 ?DO  I TO kk
                                   ...
                                   nb jj + jj ?DO c{{ J     I    }} DUP DF@+ s00 DF!
                                                  ...
                                                  a{{ J    kk }} TO 'a
                                                  b{{ kk    I }} TO 'b
                                                  nb kk + kk ?DO  'b 'a do-WARNER() to 'a to 'b
                                                            LOOP
                                                  s11 DF@ DF!
                                                  s10 DF@ DF!
                                                  s01 DF@ DF!
...
\ 32align makes DO-MAENO a bit faster. With 32align 10.79 MFlops,
\ without 32align only 9.34 MFlops.
\ Note: Most words are not affected by 32align

32align
code  DO-MAENO ( a1 a2 a3 -- )
        a! DF@  FDUP    a@ DF@+ F* addr t0 DF+!
        FDUP               DF@+ F* addr t1 DF+!
        FDUP               DF@+ F* addr t2 DF+!
        DF@  F* addr t3 DF+!
        DF@
        FDUP            a@ DF@+ F* addr t4 DF+!
        FDUP               DF@+ F* addr t5 DF+!
        FDUP               DF@+ F* addr t6 DF+!
        DF@  F* addr t7 DF+!
    next,
    end-code
