...
0 [IF] =================================================================
<A HREF="lz77.txt"> <SMALL>Get TEXT</SMALL></A>
...
sets. It also uses `READ-FILE` and `WRITE-FILE` from the File
Access word set. It presumes that `R/O`, `R/W`, `W/O`, `BIN`,
`OPEN-FILE`, `CREATE-FILE`, and `TO`, will be used
appropriately for file assignment. The program also uses
`NOT`, which can be equivalent to either `0=` or `INVERT`.
...
output is hardly acceptable. `Read-Char` used here is painfully
defined with `READ-FILE`.

A better definition would be to buffer input of many characters at
a time.  The same should be done for `Write-String`.

The spelling of a word is consistent and no words are distinguished by
a difference of case. It is immaterial whether letter-case in your
system is significant or insignificant.
...
: Write-String-in-File   ( adr slen fileid -- )  WRITE-FILE Checked ;
: Write-Char-ansi        ( adr - )         1 Out-File Write-String ;

variable Wpointer

' Read-Char-ansi is Read-Char
' Write-Char-ansi is Write-Char

' Write-String-in-File is Write-String
...
\      2016-01-09 Adapted for Win32Forth and multifile handling by J.v.d.Ven
...
\  to NIL.  Note there are 256 trees.
      ...
      : N-MOD   s" 4095 AND" evaluate ; immediate  \  Modulo N
...
: Encode                                  ( -- )
      ...
      BEGIN                                     ( s r)
            ...
            Last-Match-Length @ dup 0 DO        ( s r n)
                  ...
                  dup 0< IF  2DROP I LEAVE  THEN          \ -1 = done
...
: Decode                                  ( -- )
      ...
      BEGIN
            ...
            IF
                  ...
                  over Text-Buf + C!            ( . r)
                  dup Text-Buf +    Write-Char
                  1+  N-MOD
            ELSE
                  In-File Read-Char             ( . r i)
                  dup 0< IF  3DROP  EXIT THEN
                  In-File Read-Char             ( . . i j)
                  dup 0< IF  4DROP  EXIT THEN
...
\ Test section adapted for Win32Forth

\ Put a file named test.txt in your directory

    S" TEST.LZ" DELETE-FILE DROP

    S" test.txt" R/O OPEN-FILE Checked TO In-File
    S" test.lz" W/O  CREATE-FILE Checked TO Out-File
    Encode
    cr dir test.*

    S" test.txt" DELETE-FILE DROP

    cr dir test.*
    S" test.lz" R/O  OPEN-FILE Checked TO In-File
    S" test.txt" W/O CREATE-FILE Checked TO Out-File
...
Out: 202697
Saved: 62%
Elapsed time: 00:00:24.786

Directory of: test.*
     202,697 test.lz          538,112 test.txt
2 Files displayed, using 740,809 bytes of disk.

Directory of: test.*
     202,697 test.lz
1 Files displayed, using 202,697 bytes of disk.

Decoding: Elapsed time: 00:00:21.150
Directory of: test.*
     202,697 test.lz          538,112 test.txt
2 Files displayed, using 740,809 bytes of disk. ok
