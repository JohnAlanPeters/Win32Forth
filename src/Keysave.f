\ $Id: Keysave.f,v 1.7 2008/08/18 20:44:26 camilleforth Exp $

\ KeySave.f             Save All Keystrokes             Tom Zimmer

\ Slight modifications to KeySave.f for the new console - Rod
\ There was problem with repeat-amacro { parent \ repeat$ -- } crashing in WinEd
\ which I solved by changing  32 localAlloc: repeat$  to  MAXSTRING localAlloc: repeat$
\ I don't know why this is necessary, there could be problems elsewhere with localAlloc:
...
: "playkeys     ( a1 n1 -- )
                ...
                0 to play0cnt
\                0x0 pushkey                     \ kick start playkeys - does nothing in the new console
...
: _paste-load   ( -- )
\ *G Copy text from the clipboard to the console window and compile it.
...
: copy_console  { \ gblhndl gblptr b/l l/s len -- }
\ *G Copy text to Windows clipboard
        marked? 0=
        if      beep EXIT
        then
        conhndl call OpenClipboard 0=
...
: cut_console   ( -- )
\ *G Cut the complete text from the console window to the clipboard.
                marked?
                if      beep
                else    mark_all
                        copy_console
