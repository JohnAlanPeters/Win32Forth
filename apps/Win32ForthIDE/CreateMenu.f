...
create tpad maxstring allot	\ temporary pad
...
:M WindowStyle: ( -- style )
                WindowStyle: super
        [       TVS_HASLINES nostack1
                TVS_HASBUTTONS      or
                TVS_DISABLEDRAGDROP or
                TVS_SHOWSELALWAYS   or
                TVS_LINESATROOT     or
                TVS_EDITLABELS      or
                WS_BORDER           or ] LITERAL or
                ;M
...
:m Rename:      { str cnt -- }
                ...
                itemid: SelectedItem IDM_MENUBAR =
                if	str cnt SetMenuName: self
                then	str cnt asciiz to psztext
...
: (OnMenuItem)  { mitem -- }
                ...
                if      s" MenuSeparator" append&crlf
                        ...
                        if      s" :MenuItem " tpad place tpad +place s"  " tpad +place tpad count
...
: $:menuitem    ( -- )
                ...
                $menuitem
                tpad count setmenuname: ThisItem ;
...
: DoMenuCommand { mid obj -- }
        ...
        case
            ...
            GetID: btnClear     of  tpad 0 2dup SetText: txtName
...
\ This means that .mdf files are actually loadable from Win32Forth
            GetID: btnSave      of  hwnd Start: SaveMDFDlg count ?dup
                                    if      tpad place
                                            tpad count ".ext-only" nip 0=
                                            if      s" .mdf" tpad +place     \ add default extension
                                            then    tpad count SetName: MenuFile
...
:M WM_NOTIFY    ( h m w l -- f )
        dup @ GetHandle: TheMenu =
        if Handle_Notify: TheMenu
        else false
        then ;M
...
: CreateMenu        ( -- )
                GetHandle: MainWindow SetParentWindow: frmCreateMenuForm
                Start: frmCreateMenuForm ; IDM_FORM_CREATEMENU SetCommand
