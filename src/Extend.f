\ $Id: Extend.f,v 1.37 2015/12/30 11:39:40 jos_ven Exp $


\ Extend the kernel with the new console

cr .( Loading Extend.f : extend nucleus... ) cr

sys-FLOAD src\comment.f
    FLOAD src\numconv.f          \ general number conversions
    FLOAD src\console\Console1.f \ Console I/O part 1 - reduced
...
sys-FLOAD src\interpif.f         \ interpretive conditionals
    ...
    FLOAD src\WinVersion.f       \ Windows Versioning

    FLOAD src\paths.f            \ multi path support words
sys-FLOAD src\NumberAlloc.f      \ Allocates numbers without the use of C, or ,
...
sys-FLOAD src\struct200X.f       \ 200X structure words
...
\ sys-fload src\Pre-save.f        \ DON'T use sys-fload but the following instead
in-system Require src\Pre-save.f in-previous \ MUST be conditional
...
' ForthBoot SAVE Win32for           \ save Win32For.EXE
...
[if]
        ...
        s" \Win32for.exe" pad +place
        pad count "path-file drop AddToFile

        CREATEPROCESS_MANIFEST_RESOURCE_ID RT_MANIFEST s" src\res\Win32For.exe.manifest" "path-file drop  AddResource
 ...
 [then]
        Require Checksum.f
        s" Win32for.exe"  current-dir$ count makeabsolutepath count (AddCheckSum)
