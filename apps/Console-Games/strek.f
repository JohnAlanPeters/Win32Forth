...
\   2006-10-15  new random number generator, additional
\               logic for ShieldControl and DamageControl when
\               docked at starbase.  km
\   2006-10-21  ported to win32forth (dbu)
\
...
: KlingonsShoot ( -- )
	...
	k{{ I 3 }} @ 0> IF
	    ...
	    k{{ I 1 }} ?  ." ,"  k{{ I 2 }} ?
            s @ 0 <= IF  END_SHIP_DESTROYED EndOfGame EXIT  THEN
...
: ShortRangeScan ( -- | Short range sensor scan )
    ...
    d{ 2 } f@ f0< IF  ." *** SHORT RANGE SENSORS ARE OUT ***"
...
: DamageControl  ( -- )
	...
	d0 @ IF
	    ...
	    d3 f@ 100e f* ftrunc>s 100 / .  ." STARDATES" CR
...
create-turnkey? [IF]
    \ ??? before making it a TURNKEY, the programm should be enhanced, so that
    \ we can enter without typing "newgame" and exit (no Quit command)
    ...
    &forthdir count &appdir place  \ create in the Forth-directory
    ConsoleHiddenBoot              \ boot with hidden console, the prog customize it
    ' strekSaved SAVE StarTrek.exe \ create the application
