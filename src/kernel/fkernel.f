\ $Id: fkernel.f,v 1.105 2016/01/08 16:05:36 jos_ven Exp $
\ WIN32FORTH KERNEL
\ Andrew McKewan
\ March 1994
...
DEFER BYE ' k_BYE IS BYE   \ Close with the console 'X' to save the window size
DEFER MS  ' DROP  IS MS
...
\ [malloc_next][heapaddress][mem_type][ 0/class ] [malloced_memory][extra_cells]
\                                       ^        |
\            contains 0 for allcated memory      * returns this address on allocate *
\            class pointer for objects           this is the "address"
\            created with new> see class.f
\ Changes:
...
: allocate      ( u -- addr fl )                \ ansi version of malloc
                ...
                if      drop 0 TRUE             \ error, fl=true
                        ...
                        link>mem 0 over cell- ! \ point at real mem and zero preceeding cell
                        FALSE
...
: free          ( addr -- f1 )                  \ release the memory pointer
                ...
                mem>link                        \ point at true address
...
: close-file ( fileid -- ior )                     \ Does not save it
...
: ("HEADER) ( a1 n1 -- )
            ...
            IF >SYSTEM ELSE >APPLICATION THEN \ then build the header in the system space  otherwise in application space
...
\ (which may not be the same space as the header).
            align HERE dup LatestXT ! CFAPTR!
            ;
...
\ ** is found first in a full dictionary search).
                ...
                CFAPTR! dup latestXT ! reveal ;            \ set the cfa pointer of header
...
: Sys-warn-does?  ( newcode destxt -- )
                suppress @ if suppress off 2drop else
                SYS-ADDR? 0= SWAP
                SYS-ADDR? AND SYS-WARNING? AND
                IF WARN_SYSWORD3 WARNMSG THEN then ;
...
: :NONAME       ( -- xt )       \ start a headerless colon definition
                ALIGN HERE DUP ANON ! :COLONDEF dup latestXT ! ;
...
DEFER WINPAUSE      ( -- )  \ release control to OS for a moment, pump the message loop
...
: EXIT          ( -- )
                ?COMP ?:M -IF COMPILE, ELSE DROP PARMS IF COMPILE UNNESTP ELSE
                COMPILE UNNEST THEN THEN   ; IMMEDIATE
...
: ;             ( -- )
                ?COMP ?CSP ANON @  0= ?:M 0= AND IF REVEAL THEN ANON OFF
\                PARMS IF      COMPILE UNNESTP ELSE    COMPILE UNNEST THEN
                [COMPILE] EXIT
                FALSE TO ?:M  [COMPILE] [ PARMS-INIT DO-;CHAIN suppress off ; IMMEDIATE
