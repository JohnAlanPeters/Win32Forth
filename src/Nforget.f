\ $Id: Nforget.f,v 1.5 2012/03/03 09:15:14 georgeahubert Exp $
\ NFORGET.F
...
: sys-trim?     ( nfa addr -- f )       \ TRUE if forgetting nfa removes addr from dict
        tuck sys-origin dup sys-size + between
        if                              \ addr is IN-SYSTEM or TRANSIENT
                n>link sys-here         \ so use nfa as trim address
...
: trim-chains   ( nfa -- nfa )          \ trim down the chain linked list
        chain-link
        begin   @ ?dup
        while   2dup cell+ @ full-trim
        repeat  dup chain-link full-trim
        sys-chain-link
        begin   @ ?dup
        while   2dup cell+ @ full-trim
        repeat  dup sys-chain-link full-trim ;
...
: trim-handles  ( nfa -- nfa )          \ trim handles list
        dup handles-list full-trim ;

forget-chain chain-add trim-handles
...
post-forget-chain chain-add trim-WinLibs
...
post-forget-chain chain-add trim-WinProcs
...
: (forget) ( cfa -- )   \ assumes count follows name
        ...
        do      dup i @ trim?
                ...
                then
                cell +loop
        dup N>LINK sdp !          \ update SYS-HERE
...
: forget ( -<name>- )
        bl word count
        current @ search-wordlist ?missing ( cfa ) (forget) ;
...
: mark  ( -<name>- )
        >application create save-source sys-warning? sys-warning-off
        do-mark to sys-warning? application> ;
